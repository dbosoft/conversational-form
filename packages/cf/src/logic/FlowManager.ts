...
export class FlowManager implements IFlowManager {
	...
	*/
	...
	private userInputSubmitCallback?: (e: CustomEvent) => void;

	public get currentTag(): IDomTag | ITagGroup {
	...
	public startFrom(indexOrTag: number | IDomTag | ITagGroup, ignoreExistingTags: boolean = false) {
	...
	*/
		...
		if (this.savedStep != -1) {
			...
			for (var i = 0; i < this.tags.length; i++) {
				...
				if (tag !== this.currentTag && tag.hasConditions()) {
					...
					if (this.currentTag.name && tag.hasConditionsFor(this.currentTag.name)) {
	...
	public addTags(tags: Array<IDomTag | ITagGroup>, atIndex: number = -1): Array<IDomTag | ITagGroup> {
		// used to append new tag
		if (atIndex !== -1 && atIndex < this.tags.length) {
			const pre: Array<IDomTag | ITagGroup> = this.tags.slice(0, atIndex)
			const post: Array<IDomTag | ITagGroup> = this.tags.slice(atIndex, this.tags.length)
	...
	*/
	public editTag(tag: IDomTag | ITagGroup): void {
	...
	private setTags(tags: Array<IDomTag | ITagGroup>) {
		this.tags = tags;
